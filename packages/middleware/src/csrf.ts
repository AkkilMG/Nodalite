 ...
 * client must echo it back in a header or body field. If they don't match,
 * the request is rejected.
 *
 * Safe methods (GET, HEAD, OPTIONS, QUERY) are skipped by default since they
 * should never cause state changes.
 *
 * ```ts
 * app.use("*", csrf());
 * ```
...
export function csrf(opts: CsrfOptions = {}): Middleware {
  ...
  return async (c, next) => {
    ...
    // Seed the cookie on safe methods if not present
